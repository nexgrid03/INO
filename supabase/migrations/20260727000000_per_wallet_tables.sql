-- ============================================================================
-- INO - One table per wallet (replaces the single `public.documents` table)
-- ----------------------------------------------------------------------------
-- WHY: every wallet stores a different shape of record (a property has a
-- registration number and an EMI, a card has a last4 and an expiry, a
-- credential has a username and a secret). Cramming all of them into one wide
-- `documents` table meant most columns were NULL most of the time and no
-- column could be made NOT NULL.
--
-- WHAT THIS DOES
--   1. DROPS `public.documents` (destructive - read the warning below).
--   2. Creates one table per built-in wallet, each with the SAME 13 core
--      columns plus its own wallet-specific columns.
--   3. Adds `public.wallets` - a registry of every wallet that has a table.
--   4. Adds `public.create_custom_wallet(name, icon, color)` so a user-created
--      wallet ALSO gets its own table, at runtime, with RLS already applied.
--   5. Recreates `public.documents` as a READ-ONLY VIEW that unions the core
--      columns of every wallet table, so the existing share Edge Function and
--      `document_shares` ownership check keep working unchanged.
--
-- ⚠️  DESTRUCTIVE: step 1 deletes every existing document row. Take a backup
--     first if the data matters:
--         create table public.documents_backup_20260727 as
--           select * from public.documents;
--     Storage objects in the `documents` bucket are NOT touched.
--
-- ⚠️  APP CHANGE REQUIRED: `DocumentRepository` (lib/repositories/document_
--     repository.dart) writes to `documents`. The view is read-only, so
--     INSERT / UPDATE / DELETE must be re-pointed at the per-wallet tables
--     before this migration ships. Reads that filter on `wallet` keep working.
--
-- Safe to re-run. Core Postgres only (gen_random_uuid is built in on PG13+).
...
-- ----------------------------------------------------------------------------
-- 0. Drop the old monolithic table (and the view, if this is a re-run).
-- ----------------------------------------------------------------------------
...
-- Turns a wallet label into a safe table name: "My Pets 🐾" -> "w_my_pets".
...
-- ----------------------------------------------------------------------------
-- 2. Wallet registry
--    One row per wallet that has a physical table. `slug` IS the table name.
...
create table if not exists public.wallets (
  ...
  label        text not null unique,      -- what the UI shows, e.g. "Property Wallet"
  kind         text not null default 'custom'
               check (kind in ('builtin', 'custom', 'system')),
  icon_key     text,
  color_value  bigint,                    -- ARGB int from kWalletAccentValues
...
-- Every signed-in user needs to see the catalogue; only the SECURITY DEFINER
-- function below may write to it.
...
-- ----------------------------------------------------------------------------
-- 3. The table factory
--    Creates a wallet table with the 13 CORE columns, its indexes, RLS and the
--    four owner-only policies. Wallet-specific columns are bolted on after.
--
--    CORE COLUMNS (identical in every wallet table, so the view in step 6 and
--    generic code like search / export can treat any wallet uniformly):
...
-- Registers a wallet and builds its table in one call.
...
-- ----------------------------------------------------------------------------
-- 4. The built-in wallets - core skeleton first, then wallet-specific columns.
-- ----------------------------------------------------------------------------
...
alter table public.w_identity_wallet
  ...
  add column if not exists id_type            text,   -- aadhaar | pan | passport | licence | voter | other
...
alter table public.w_insurance_wallet
  ...
  add column if not exists policy_type        text,   -- life | health | motor | travel | home | other
  ...
  add column if not exists premium_frequency  text,   -- monthly | quarterly | yearly | single
...
alter table public.w_health_wallet
  add column if not exists record_type      text,   -- prescription | report | vaccination | bill | insurance
...
alter table public.w_investment_wallet
  ...
  add column if not exists institution      text,   -- broker / AMC / bank
  add column if not exists account_number   text,
  add column if not exists units            numeric(20, 6),
  add column if not exists purchase_price   numeric(18, 4),   -- per unit
...
--     `secret` MUST hold a client-side-encrypted value. Postgres, PostgREST
--     logs and any future dashboard viewer all see this column in plaintext
--     otherwise - RLS protects it from other users, not from operators.
...
--      '__ino_share_cache__'`; now its own table so it never pollutes a real
--      wallet. Must stay in the `documents` view: the share Edge Function
--      serves these rows by id.
...
--        });
--    …and gets back the slug (= table name) to read and write from.
--
--    A custom table only ever gets the 13 core columns. Anything wallet-shaped
--    beyond that goes in `notes` or gets a hand-written ALTER TABLE.
-- ----------------------------------------------------------------------------
...
begin
  ...
  -- Reject anything that is not a plain wallet name. This function creates
  -- SCHEMA OBJECTS, so the input is validated before it reaches the factory
  -- rather than relying on quoting alone.
  ...
  notify pgrst, 'reload schema';   -- make the new table visible to the REST API
  return v_slug;
end;
$fn$;
...
-- may share it). It deletes the caller's own rows; RLS scopes the delete.
...
-- in it). Not granted to `authenticated` on purpose - run it from the SQL
-- editor when you are sure the wallet is unused.
...
--
--    Unions the core columns of every registered wallet table and adds back the
--    `wallet` label column, so:
--      * supabase/functions/share/index.ts  (select id, name, file_path …)
--      * the ownership check in 20260704000000_document_shares.sql
--      * global search / dashboards / export, which read across all wallets
--    keep working against one relation.
--
--    security_invoker = on -> the underlying tables' RLS decides what a caller
--    sees. The service role used by the Edge Function bypasses RLS as before.
--
--    Rebuilt automatically whenever a custom wallet is created or dropped.
-- ----------------------------------------------------------------------------
